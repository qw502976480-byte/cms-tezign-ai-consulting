...
// FIX: Import `useSearchParams` from `next/navigation`.
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200/75 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                    ...
                    />
                     <input 
                        ...
                        className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 transition-all"
                        value={country}
                ...
                <div className="flex flex-col md:flex-row flex-wrap gap-4 items-center">
                    <div className="flex items-center gap-2 text-sm text-gray-500">
                     ...
                     <div className="flex items-center gap-2 text-sm text-gray-500">
                        <label>最近登录:</label>
                        <input type="date" value={searchParams.login_from || ''} onChange={(e) => updateFilter('login_from', e.target.value)} className="border border-gray-200 rounded-md px-2 py-1" />
                        <span>-</span>
                        <input type="date" value={searchParams.login_to || ''} onChange={(e) => updateFilter('login_to', e.target.value)} className="border border-gray-200 rounded-md px-2 py-1" />
                    </div>
                     <FilterDropdown 
                        label="线上沟通" 
                    ...
                    />
                    <button onClick={handleClearFilters} className="text-sm text-gray-500 hover:text-gray-900 flex items-center gap-1.5 ml-auto">
                        <Trash2 size={14} /> 清空筛选
                    </button>
                </div>
            </div>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {initialUsers.map((user) => (
                            <tr key={user.id} className="hover:bg-gray-50 transition-colors group">
                                <td className="px-6 py-4 align-top">
                                    ...
                                    <div className="text-xs text-gray-500 mt-1 pl-5 truncate" title={user.email}>{user.email}</div>
                                </td>
                                <td className="px-6 py-4 align-top">
                        ...
                        {initialUsers.length === 0 && (
                            <tr><td colSpan={7} className="text-center py-12 text-gray-400">无符合条件的用户</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
