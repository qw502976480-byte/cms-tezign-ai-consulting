...
const ResourceListItem: React.FC<ResourceListItemProps> = ({ item }) => (
  <li className="flex items-center justify-between p-3.5 hover:bg-gray-50 rounded-lg transition-colors">
    <div className="flex-1 pr-4">
      <p className="font-medium text-sm text-gray-800 truncate" title={item.title}>{item.title}</p>
    </div>
    <div className="flex items-center gap-3">
      {item.status !== 'unknown' && (
         <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium border whitespace-nowrap ${
      ...
      {item.slug && (
        <Link href={`/admin/resources/${item.id}/edit`} className="text-gray-400 hover:text-gray-900 transition-colors">
...
export default async function HomepageSlotsPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">首页配置 (Homepage)</h1>
        <p className="text-sm text-gray-500 mt-2 max-w-2xl">
          此页面仅用于展示「最新动态」模块的当前配置。如需修改，请前往
          <Link href="/admin/resources" className="text-gray-900 font-medium underline hover:text-black mx-1">内容资源</Link>
          进行操作。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-gray-900">最新动态 · 轮播位</h2>
          </div>
        ...
        <div className="space-y-4">
           <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-gray-900">最新动态 · 固定位</h2>
          </div>
          <div className="bg-white rounded-xl border border-gray-200">
            <ul className="divide-y divide-gray-100 p-1">
               {fixedItems.length > 0 ? (
